...
def deployed_contract():
    ...
    nft_contract.setTokenIdToMerkleRoot(Token_ID.GIFT_OF_HADES.value,
                                      '0x021a103e62c519a6bd4a9f46959b21a5c06255736055a2c943bacdd7eadfe6fc')
    nft_contract.setMaxTokenSupply(Token_ID.GIFT_OF_HADES.value, 55)
    nft_contract.togglePrivateSaleOpen(Token_ID.GIFT_OF_HADES.value)
    return nft_contract


def test_can_mint_successfully(deployed_contract):
    deployed_contract.privateMint(Token_ID.GIFT_OF_HADES.value, accounts_merkle_proof[0])
    assert deployed_contract.balanceOf(accounts[0], Token_ID.GIFT_OF_HADES.value) == 1
    assert deployed_contract.exists(Token_ID.GIFT_OF_HADES.value)
...
def test_cannot_mint_before_open_to_sale():
    ...
    with reverts("dev: sale is not open currently"):
        nft_contract.privateMint(Token_ID.GIFT_OF_HADES.value, accounts_merkle_proof[0])



def test_cannot_mint_more_then_cap_goh():
    nft_contract = TheAscendantsPass.deploy(accounts[0], {"from": accounts[0]})
    nft_contract.setTokenIdToMerkleRoot(Token_ID.GIFT_OF_HADES.value,
                                        '0x021a103e62c519a6bd4a9f46959b21a5c06255736055a2c943bacdd7eadfe6fc')
    nft_contract.setMaxTokenSupply(Token_ID.GIFT_OF_HADES.value, 2)
    nft_contract.togglePrivateSaleOpen(Token_ID.GIFT_OF_HADES.value)
    nft_contract.privateMint(Token_ID.GIFT_OF_HADES.value, accounts_merkle_proof[0],
                                  {"value": web3.toWei("0.2", "ether")})
    nft_contract.privateMint(Token_ID.GIFT_OF_HADES.value, accounts_merkle_proof[1],
                                  {"from": accounts[1], "value": web3.toWei("0.2", "ether")})

    with reverts("dev: max token supply minted"):
        nft_contract.privateMint(Token_ID.GIFT_OF_HADES.value, accounts_merkle_proof[2],
...
def test_cannot_mint_if_sale_is_not_open():
    ...
    nft_contract.setTokenIdToMerkleRoot(Token_ID.GIFT_OF_HADES.value,
                                        '0x021a103e62c519a6bd4a9f46959b21a5c06255736055a2c943bacdd7eadfe6fc')
    nft_contract.setMaxTokenSupply(Token_ID.GIFT_OF_HADES.value, 555)
    with reverts("dev: sale is not open currently"):
        nft_contract.privateMint(Token_ID.GIFT_OF_HADES.value, accounts_merkle_proof[0])

def test_cannot_if_not_on_whitelist_no_proof(deployed_contract):
    with reverts("dev: not on the whitelist"):
        deployed_contract.privateMint(Token_ID.GIFT_OF_HADES.value, [], {"from": accounts[9]})

def test_cannot_if_not_on_whitelist_wrong_proof_but_on_wl(deployed_contract):
    with reverts("dev: not on the whitelist"):
        deployed_contract.privateMint(Token_ID.GIFT_OF_HADES.value, accounts_merkle_proof[0], {"from": accounts[1]})
