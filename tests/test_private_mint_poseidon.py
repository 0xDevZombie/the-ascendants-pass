...
def test_cannot_mint_before_open_to_sale():
    ...
    with reverts("dev: sale is not open currently"):
        nft_contract.privateMint(Token_ID.GIFT_OF_POSEIDON.value, accounts_merkle_proof[0],
                                 {"value": web3.toWei("0.2", "ether")})
...
def test_cannot_mint_more_then_cap_gop():
    nft_contract = TheAscendantsPass.deploy(accounts[0], {"from": accounts[0]})
    nft_contract.setTokenIdToMerkleRoot(Token_ID.GIFT_OF_POSEIDON.value,
                                        '0x021a103e62c519a6bd4a9f46959b21a5c06255736055a2c943bacdd7eadfe6fc')
    nft_contract.setMaxTokenSupply(Token_ID.GIFT_OF_POSEIDON.value, 2)
    nft_contract.togglePrivateSaleOpen(Token_ID.GIFT_OF_POSEIDON.value)
    nft_contract.privateMint(Token_ID.GIFT_OF_POSEIDON.value, accounts_merkle_proof[0],
                             {"value": web3.toWei("0.2", "ether")})
    nft_contract.privateMint(Token_ID.GIFT_OF_POSEIDON.value, accounts_merkle_proof[1],
                             {"from": accounts[1], "value": web3.toWei("0.2", "ether")})

    with reverts("dev: max token supply minted"):
        nft_contract.privateMint(Token_ID.GIFT_OF_POSEIDON.value, accounts_merkle_proof[2],
...
def test_cannot_if_not_on_whitelist_no_proof(deployed_contract):
    with reverts("dev: not on the whitelist"):
        deployed_contract.privateMint(Token_ID.GIFT_OF_POSEIDON.value, [],
                                      {"from": accounts[9], "value": web3.toWei("0.2", "ether")})
